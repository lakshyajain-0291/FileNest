...
// Add these methods to your existing Node struct
...
// Ping sends a Kademlia PING message to the specified peer
func (n *Node) Ping(peerID peer.ID) error {
	...
	if response.Type != PONG {
		return fmt.Errorf("unexpected response type: %v, expected PONG", response.Type)
	}

	return nil
}

// PingAll pings all known contacts in the routing table
func (n *Node) PingAll() {
	...
	for _, contact := range contacts {
		go func(peerID peer.ID) {
			err := n.Ping(peerID)
			if err != nil {
				log.Printf("Ping to %s failed: %v", peerID, err)
			} else {
				log.Printf("Ping to %s successful", peerID)
			}
		}(contact.ID)
	}
}
...
// Add this function to generate message IDs
...
func (n *Node) Bootstrap(bootstrapPeers []peer.ID) error {
	...
	for _, peerID := range bootstrapPeers {
		...
		for attempt := 0; attempt < 3; attempt++ {
			_, pingCancel := context.WithTimeout(n.ctx, 5*time.Second)
			err = n.Ping(peerID)
			pingCancel()
